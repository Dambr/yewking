...
Для линеаризации выражений матрицы $f_{im}\big((Q \cdot \dot{F})^{T}\big)$ требуется $l \cdot n$ дополнительных бинарных переменных $\gamma$, которые удобно представить в виде матрицы $\Gamma_{l \times n} = ||\gamma_{i, j}||$. Тогда выражение $\displaystyle f_{im}(\sum_{i = 1}^{m}\big(q_{\tilde{n}, i} \cdot \sum_{j = 1}^{k}\beta_{k \cdot (i - 1) + j, \tilde{l}}\big))$ заменяется на $\gamma_{\tilde{l}, \tilde{n}}$, а модель дополняется ограничениями:
