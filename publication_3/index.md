...
Вектора $R_{i}$ могут быть рассмотрены как входные данные работы графа.
...
Матрица $F_{l \times m} = ||f_{i, j}||$ описывает полезные к поставке файлы исходного кода в каждой из $l$ комбинаций. Номер строки соответствует номеру комбинации. Номер стобца соответствует индексу файла. Если в $i$-й комбинации $j$-й файл полезен $f_{i, j} > 0$. Иначе $f_{i, j} = 0$. 
...
$$F'' \gets F' \cdot D \gets F \cdot D \cdot D \gets F \cdot D^{2}$$
...
Матрица $\hat{F}_{l \times m} = ||\hat{f}_{i, j}||$ описывает полезные файлы с учетом разрешения их зависимостей. Аналогично матрице $F$ в нем номер столбца соответствует индексу файла. Для вычисления его значений необходимо просуммировать матрицу полезных файлов и каждую из матриц разрешения зависимостей.
...
Для матрицы $\hat{F}$ так же как и для значений матрицы $F$ характерно, что отсутствует ограничение на положительное значение.
...
Для описания алгоритма определения $\dot{R}$ необходимо ввести вспомогательные функции:
...
Алгоритм определения $\dot{R}$ следующий:

2. Вычислить состав файлов исходного кода, реализующих полезные требования:
$$F \gets R \cdot T$$

3. Разрешить их зависимости:

$$\hat{F} \gets F \cdot \sum^{m}_{j = 0} D^{j}$$

*Примечание: для программной реализации удобно применить к матрице $\hat{F}$ функцию $f_{in}$.*

4. Определить поставляемые составы плагинов:
...
### Рассчет стоимости комплектаций
