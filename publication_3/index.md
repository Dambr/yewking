...
В настоящее время решение оптимизационных задач - востребованное направление в сфере информационных технологий, например для программного обеспечения (ПО) [1]. Решение таких задач актуально не только для оптимизации технической составляющей программных решений [2], но и экономической их составляющей. Для технической составляющей актуальными являются вопросы оптимизации работы с разделяемыми ресурсами, организация распределенных вычислений и т.д. Для экономической составляющей важны такие аспекты как оптимизация временных издержек на разработку, сопровождение и послепродажное обслуживание.
...
Для оценивания стоимости существуют различные подходы и на ее конечную величину влияют многие факторы. В настоящей работе в качестве определяющего фактора выделены функциональные требования: их состав и стоимость каждого из них.
...
Имея сведения об объеме стоимостных издержек каждого из реализованных требований, можно оценить суммарную их величину применимо для всей версии ПО в целом.
...
2. файлы исходного имеют зависимости друг на друга и для осуществления поставки все зависимости должны быть разрешены;
3. плагины содержат файлы исходного кода и обеспечивают интеграцию с плагинной средой выполнения.

Сущности и связи между ними образуют граф [9, 10, 11]. Используя его, можно определить комплектации плагинов. Каждая комплектация реализует конечный и однозначно определяемый состав требований.
...
Каждый файл может быть необходим для реализации одного или нескольких требований, а каждое требование реализовано в одном или нескольких файлах. Допускается, что могут быть файлы, которые не задействованы в реализации хотя бы одного требования, но каждое требование должно быть реализовано. За признак реализованности положена сумма соответствующих элементов матрицы, она должна быть равна $1$. Вследствие этого, элементы матрицы $Q$ удовлетворяют следующим условиям:
...
Такие связи можно представить в виде квадратной матрицы бинарных отношений $D_{m \times m} = ||d_{i, j}||$. В ней номера строк и столбцов соответствуют индексам файлов исходного кода. Если $i$-й файл имеет зависимость на $j$-й, то $d_{i, j} = 1$. Иначе $d_{i, j} = 0$. Полагается, что файл не зависит сам от себя, поэтому значения элементов на главной диагонали равны $0$.
...
Заметим, что максимальная глубина зависимостей не может превышать значение $m$, поэтому достаточно разрешить зависимости до этого значения глубины.
...
$$\hat{F} \gets F + F' + F'' + \cdots + F^{(m)} = F + F \cdot D + F \cdot D^{2} + \cdots + F \cdot D^{m} = F \cdot (E + \sum^{m}_{i = 1}D^{i}) = F \cdot (D^{0} + \sum^{m}_{i = 1}D^{i}) = F \cdot \sum^{m}_{i = 0}D^{i}$$
...
Комплектации плагинов для каждой комбинации полезных требований образуют матрицу бинарных отношений $P_{l \times k} = ||p_{i, j}||$. Номер строки соответствует номеру комплектации. Номер столбца соответствует индексу плагина. Если $j$-й плагин должен быть включен в $i$-ю комплектацию $p_{i, j} = 1$. Иначе $p_{i, j} = 0$.
...
1. Лицо, принимающее решение, формирует комбинации полезных требований $R_{l \times n}$.
...
Примечательно, что ограничения значений матриц $X$ и $P$ обеспечивают бинарность отношений $\dot{F}$. Благодаря этому отсутствует необходимость дополнительной обработки ее значений, например, с использованием функции $f_{in}$.

### Реализованные требования

Реализованным называется такое требование, для которого поставлен весь задействованный в реализации код. Состав реализованных требований влияет на стоимость комплектации.
...
Величины изменения стоимости реализации требований описывает матрица $C_{n \times n} = ||c_{i, j}||$. В ней номера строк и столбцов соответствуют индексам требований. На ее главной диагонали значениями являются стоимости реализации требования без учета реализации смежных. Прочие элементы в строке указывают на сколько изменится стоимость реализации требования, если будет реализовано соответствующее смежное требование.
...
Стоимость $i$-го требования в комплектации $\tilde{l}$:

$$\sum_{j = 1}^{n}(c_{i, j} \cdot \dot{r}_{\tilde{l}, j})$$

Данное выражение описывает стоимость требования, как если бы оно входило в комплектацию. Однако даже если само оно не реализовано по этой формуле оно может иметь ненулевую стоимость. Чтобы нивелировать это обстоятельство данное выражение, необходимо домножить на признак реализованности $i$-го требования:

$$\dot{r}_{\tilde{l}, i} \cdot \sum_{j = 1}^{n}(c_{i, j} \cdot \dot{r}_{\tilde{l}, j})$$
...
$$\hat{F} \gets R \cdot Q \cdot \sum^{m}_{i = 0} D^{i} =
\begin{pmatrix}
...
1. $\dot{a}$ не может быть отрицательным;
2. $\alpha \in \{0; 1\}$.

Следовательно $\alpha$ может превышать или быть равной $\dot{a}$ только при $\dot{a} \in [0; 1]$. При $\dot{a} \in (1; \infty)$ верно всегда, что $\dot{a} > \alpha$. Поэтому достаточно рассмотрены $4$ диапазона значений $\dot{a}$, которые указаны в таблице. В зависимости от диапазона переменная $\alpha$ принимает значение $0$ или $1$ согласно лианиризирующей функции. Дизъюнкция выражений вводимых ограничений показывает корректность значения $\alpha$ от текущего значения $\dot{a}$.

Корректность вводимых ограничений обоснована истинностью значений $\alpha$ в каждом из диапазонов $\dot{a}$.

Для будущей программной реализации строгое неравенство должно быть заменено нестрогим. С этой целью вводится постоянная величина $1 / M$ - условно малое число. Тогда, выражение вида $a < b$ будет записано как $a + 1 / M \le b$.
...
Для линеаризации выражений требуется $l \cdot n$ дополнительных бинарных переменных $\gamma$, которые удобно представить в виде матрицы $\Gamma_{l \times n} = ||\gamma_{i, j}||$. Тогда выражение $\displaystyle f_{im}(\ddot{a})$ заменяется на $\gamma_{\tilde{l}, \tilde{n}}$, а модель дополняется ограничениями:
...
Альтернативой решения оптимизационной задачи методом ее математической формализации является применение эвристических алгоритмов. В настоящей работе рассматривается применение генетического алгоритма [13, 14, 15, 16].
...
1. Директория включает исходные данные для проведения эксперимента в формате ".csv" и результаты его проведения в формате ".json" [20]
...
1. Результаты проведения эксперимента записываются в файл "solutions.json", структура которого представляет собой массив структур формата, содержащего сведения о:
    ...
    - названии применяемого в эксперименте MILP-решателя [19, 21] или метке конфигурации генетического алгоритма

    - значении функции оценки

    - потребном для выполнения вычислений время
...
1. Запись результата в файл "solutions.json". Данные будут дописаны в конец файла если для заданного MILP-решателя или метки конфигурации генетического алгоритма эксперимент проводится впервые. Если эксперимент проводится повторно информация будет обновлена.
...
Целью проведения экспериментов было сравнение эффективности применения генетических алгоритмов в решении оптимизационной задачи по сравнению с математической формализацией и последующим решением задачи линейного программирования. Ожидаемым результатом является вывод о корректности сформулированной гипотезы относительно разных методов решения оптимизационной задачи.
...
В экспериментах были задействованы MILP-решатели GNU Linear Programming Kit (glpk), COIN-OR branch and cut (cbc) [19], а так же генетический алгоритм в трех описанных ранее конфигурациях.
...
На рисунке приведен график зависимости среднего времени выполнения от размера модели для решателя glpk.
...
Результаты проведения экспериментов подтвердили ожидаемый экспоненциальный рост потребного времени MILP-решателям.
...
12. И.П. Богданов, В.А. Судаков, Н.Б. Топоров. Опитимизация загрузки упорядоченной совокупности летательных аппаратов // Известия РАН. Теория и системы управления. 2021. № 3. С. 57–70.
