...
Матрица $F_{l \times m} = ||f_{i, j}||$ описывает полезные к поставке файлы исходного кода в каждой из комбинаций полезных требований. Номер строки соответствует номеру комбинации. Номер столбца соответствует индексу файла. Если в рамках $i$-й комбинации $j$-й файл полезен $f_{i, j} > 0$. Иначе $f_{i, j} = 0$.
...
Матрица $\hat{F}_{l \times m}$ описывает полезные файлы с учетом разрешения их зависимостей. Аналогично матрице $F$ в ней номер строки соответствует номеру комбинации полезных требований, а номер столбца - индексу файла. Для вычисления значений матрицы необходимо просуммировать матрицу полезных файлов и каждую из матриц разрешения зависимостей. В результате вычислений элементы матрицы $\hat{F}$ могут принимать на столько большие значения, что однако для дальнейшей работы необходимо чтобы $\hat{F}$ была матрицей бинарных отношений.
...
Комплектации плагинов для каждой комбинации полезных требований образуют матрицу бинарных отношений $P_{l \times k} = ||p_{i, j}||$. Номер строки соответсвует номеру комбинации. Номер столбца соответствует индексу плагина. Если для обеспечения $i$-й комбинации $j$-й плагин должен быть включен в комплектацию $p_{i, j} = 1$. Иначе $p_{i, j} = 0$.
...
1. ЛПР формирует комбинации полезных для него требоний $R_{l \times n}$.
2. Для каждой из них определяются полезные файлы исходного кода:

$$F_{l \times m} = R \cdot T$$

3. Составляется матрица учитывающая полезные файлы и их разрешенные зависимости:

$$\hat{F}_{l \times m} = F \cdot (\sum^{m}_{i = 0}D^{i})$$

4. Определяется состав плагинов каждой из комплектаций:

$$P_{l \times k} = f_{in}(\hat{F} \cdot X)$$

### Поставляемый код

Объем поставляемого кода - это весь код, который относится к поставляемым в рамках комплектации плагинам. Его описывает матрица бинарных отношений $\dot{F}_{m \times l}$.

### Реализованные требования

### Рассчет стоимости как целевая функция
...
$$\dot{R_{i}} \cdot C \cdot \dot{R_{i}}^{T}$$

Целевая функция оптимизационной задачи - минимизация суммарной стоимости всех комплектаций:

$$min \to \sum^{l}_{i = 1} (\dot{R}_{i} \cdot C \cdot \dot{R}_{i}^{T})$$

$$\varPhi \quad \varphi \quad \Alpha \quad \alpha \quad \Beta \quad \beta \quad \Gamma \quad \gamma$$

### Линелиаризация

Сформулированная задача может быть решена при помощи MILP решателей. Целевая функция в исходном виде представляет собой нелинейное выражение. Большинство решателей не способны решать оптимизационные задачи с нелинейными выражениями. Однако данную задачу можно свести к линейному виду за счет ввода дополнительных переменных и ограничений.
