...
При реализации генетического алгоритма нет необходимости проводить линеаризацию модели. Достаточно описать функцию приспособленности и итерационно по мере работы генетического алгоритма производить вычисление ее значения. Для реализации генетического алгоритма в данной задаче используется функция приспособленности:

$$fitness = -1 \cdot \sum^{l}_{\tilde{l} = 1} (\dot{R}_{\tilde{l}} \cdot C \cdot \dot{R}^{T}_{\tilde{l}})$$

Входной параметр матрица $X$ генерируется на каждом шаге работы генетического алгоритма. Значения ее элементов могут формироваться из векторов значений, генерируемых на каждой итерации работы генетического алгоритма:

1. $\dot{X}_{1 \times m \cdot k} = ||\dot{x}||$ - вектор бинарных отношений. Трансформация его в матрицу $X$:

$$x_{i, j} = \dot{x}_{i \cdot (k - 1) + j} \quad i = \overline{1, m} \quad j = \overline{1, k}$$

2. $\ddot{X}_{1 \times m} = ||\ddot{x}||$ - вектор целочисленных значений в диапазоне $[0; k]$. Трансформация его в матрицу $X$:
