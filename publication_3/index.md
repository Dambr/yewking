...
<!-- TODO: перепроверить корректность. По-моему здесь не учитывается кол-во ограничений на x -->
...
Ранее была описана функция вычисления суммарной стоимости комплектаций. В качестве оценки может быть использовано противоположное число:

$$fitness = -1 \cdot \sum^{l}_{\tilde{l} = 1} (\dot{R}_{\tilde{l}} \cdot C \cdot \dot{R}^{T}_{\tilde{l}})$$

Для вычисления этого значения на каждой итерации должно производиться преобразование матрицы $X$ из генерируемого генетическим алгоритмом решения. Предложено $2$ варианта генерируемого решения:

1. $\dot{X}_{1 \times m \cdot k} = ||\dot{x}_{i}||$ - вектор бинарных отношений;

2. $\ddot{X}_{1 \times m} = ||\ddot{x}_{i}||$ - вектор целочисленных значений в диапазоне $[1; k]$.

Преобразование $\dot{X}$:

$$x_{i, j} = \dot{x}_{i \cdot (k - 1) + j} \quad i = \overline{1, m} \quad j = \overline{1, k}$$

Преобразование $\ddot{X}$:

$$X = Z$$

$$j = \ddot{x}_{i} \quad x_{i, j} = 1 \quad i = \overline{1, m}$$

Заметим, что трансформация $\ddot{X}$ в $X$ гарантирует выполнения условия $\displaystyle \sum_{i = 1}^{m}X_{i} = 1$, кроме того длина вектора $\ddot{X}$ меньше чем длина $\dot{X}$, что упрощает поиск оптимального решения.

Выдвинута гипотеза, что MILP решатели применять эффективно при небольших размерах модели: количества переменных и ограничений. Работа генетического алгоритма не зависит от размера модели и его применение эффективно при небольших значениях $m$ и $k$.
