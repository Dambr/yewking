...
Была проведена серия вычислительных экспериментов по решению оптимизационной задачи для различных значений коэффициента $k$ в диапазоне $[1; 50]$. Вычислительные экперименты для каждого из значений $k$ с применением обоих алгоритмов проводились по 10 раз с последующим определением максимального, минимального и среднего значений целевой функции. На риснуке приводится графическая интерпритация выполненных вычислительных экспериментов. Отдельно график изменения среднего значения вынесен на рисунок.
