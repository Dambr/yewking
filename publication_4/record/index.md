...
Поиск оптимальных значений матрицы $X$ методом полного перебора на задаче размерности представленных исходных не представляется возможным. В связи с этим использовались альтернативные способы решения оптимизационной задачи.

Reinforcement Lerning (RL) - обучение с подкреплением, является одним из лидирующих методов на основе искуссвенного интеллекта.

Еще одним популярным способом решения оптимизационных задач являются генетические алгоритмы. Они зарекомендовали себя . Популярность и мощность генетических алгоритмов подтверждается:

1. В работе [] продемонстирована их работа
2. Выполнена библиотека PyGad [] для предоставления упрощенного интерфейса взаимодействия с генетическим алгоритмом, а так же реализует самые популярные алгоритмы кроссовера, мутации и т.д.
3. Активно применяется в сочетании с другими алгоритмами машинного обучения. Например, в работе [] рассмотрен пример его использования совместно с RL.

Выбран один эвристический алгоритм и один метод машинного обучения. В качестве эвристического алгоритма был выбран генетический алгоритм, в качестве метода машинного обучения - reinforcement learning (RL, обучение с подкреплением).
